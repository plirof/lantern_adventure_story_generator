<?php
/*
220625- Extracted file must be saved with utf-8 CR+LF  9In order for you to see the line breaks in Notepad, you will have to replace the line breaks in your side with javascript (replace either \n or \r with \r\n).)
...
if(@$_GET["fileinput"]=="true") $file_input=true;
if($file_input){
...
else
{
	...
	foreach ($result_array as $line){
	  $line2=explode("|",$line);
	  list($room[],$room_desc[],$villain[],$villain_desc[],$item[],$item_desc[],$item_reward[],$item_reward_desc[],$exits[]) = $line2;
	  //if($debug1) {print_r($line);echo "=LINE<hr size=10>";}
	  if($debug1) {print_r($line2);echo "=LINE2 LOOP<hr size=10>";}
	}
}// END of if($file_input){


if($debug1) {echo "\n<h3>ROOMS </h3>";print_r($room);echo "<hr size=10>END ROOMS\n";}
...
?>
<button id="download-xml">download-xml</button>
...
document.getElementById("download-xml").addEventListener("click", function(){
   ...
   //text = text.replace(/(?:\r\n|\r|\n)/g, "\r\n");
   ///text = text.replace(/(?:\r\n|\r|\n)/g, "\r\n");
   //text = text.replace("object", 'AAAAAAAAAAAAa\r\n');
   console.log(text);
   download("lant-js.xml", text);

});

 </script>

<form action = "download.php" target="result_php_xml" method = "post" >
         <input type=submit value="DOWNLOAD Generated Lantern XML" >
</form>
