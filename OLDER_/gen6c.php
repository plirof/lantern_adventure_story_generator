...
$items_not_created_list=array();//hold here the items ids NOT created so we will link to the ID of the reward items
...
$items_not_created_list[]=$id_item;
if (!$found_item_again_check) { // if item NOT found in reward items THEN create it
$items1 = <<<ITEMS
...
$routine1='<routine name="kill_'.$cur_villain_name_no_space.'">println("You kill '.$villain[$counter].' ");
     ...
     '.$cur_item_reward_name_no_space.'.location= player;
